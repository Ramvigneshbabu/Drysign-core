...
table {
    ...
    width: 100%;
		...
		#SignedTable thead tr th:first-child:after, #CompletedDocumentsTable thead tr th:first-child:after, #OutForSignatureTable thead tr th:first-child:after{
			visibility:hidden;
			cursor: auto;
		}
      ...
      <div id ="my_pdf_viewer">
                <div id="canvas_container">
                    <div id="pdf_renderer"> </div>
						...
						<table id="SignedTable" class="display table Data_table_heading table-striped table-bordered" style="width:100%">
								<thead>
									<tr>
										...
										<th style="width: 200px"><spring:message code="label.signeddate"/></th>
										<th style="width: 150px"><spring:message code="label.comp_reqstdby"/></th>
								 ...
								 <tfoot>
									<tr>
											...
											<th><spring:message code="label.signeddate"/></th>	 
											<th ><spring:message code="label.comp_reqstdby"/></th>	
...
$('#SignedTable tbody').on( 'click', 'tr td.details-control', function () {
  ...
  if ( row.child.isShown() ) {
     ...
     // tr.removeClass('shown');
      ...
      row.child( formatDataSignTable ( row.data() ) ).show();
...
function signedPageColumnSearch(table){
	var i=0;
 $('#SignedTable tfoot th').each( function () {
        var title = $(this).text();
        console.log(title);
    if(i!=0 && i!=6){
...
function formatDataSignTable ( d ) {
	 ...
	 if(signType=='E' || signType=='G' || signType=='R'){
				...
				for(i=0; i<sn.length; i++){
					if(ss[i] == "0"){
						...
						content += '<tr><td>'+sn[i]+'</td><td>'+eID[i]+'</td><td><button type="button" class="badge badge-workSigned" style="cursor: inherit;"><spring:message code="label.comp_signd"/></button></td></tr>';
							...
							content += '<tr><td>'+sn[i]+'</td><td>'+eID[i]+'</td><td><button type="button" class="badge badge-workReject" style="cursor: inherit;"><spring:message code="label.rejected"/></button></td></tr>';
						}
						else if(ss[i] == "3"){
			...
			for(i=0; i<sn.length; i++){
				if(ss[i] == "0"){
					...
					content += '<tr><td>'+pp[i]+'</td><td>'+sn[i]+'</td><td>'+eID[i]+'</td><td><button type="button" class="badge badge-workReject" style="cursor: inherit;"><spring:message code="label.rejected"/></button></td></tr>';
				}
				else if(ss[i] == "3"){
