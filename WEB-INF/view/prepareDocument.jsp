...
<input type="hidden" id="docId" value="${docId}"/>
    ...
    <div id="loader1" class="loader2" style="display:block">
	<div class="loaderdiv">
    ...
    <nav class="navbar navbar-default inner-header">
        <div class="container-fluid">
            ...
            <div class="pull-right">
                ...
                <button id="qa" value="1" class="btn has-spinner PDF-btn-pri" style="background-color:#9b59b6;color:#fff;min-width: 110px;padding: 2px 15px;" onclick="qualityImprove(this.value);" data-toggle="tooltip" data-placement="top" ><spring:message code="label.enhanceDocument"/></button>
    ...
    <div class="container-fluid" style="margin-top: 144px;display: none;" id="success_content">
  		<div class="Content-wh-module text-center">
				   ...
				   <c:choose>
					    ...
					    <c:otherwise>
					       <a href="${prepareReturnURL}" style="margin-top: 50px;" class="btn btn-primary font-24"><spring:message code="label.ok"/></a>
					    </c:otherwise>
					</c:choose>
                ...
                <div class="pollSlider" style="z-index:3px !important;">
                    <div class="main-content" style="display: none;">
                        <div class="header-text text-center">
                            <i><spring:message code="label.dragDropPdf"/></i>
                        </div>
                        <div class="text-center margTB20" id="draggable1">
                            ...
                            <div id="signer-1" class="dragMe dragSigners btn btn-light-oragne margB20" data-signer-id="1" data-document-id="1">
                                <span class="icon-menu pull-left"></span><spring:message code="label.name"/><span class="icon-menu pull-right"></span>
                            </div>

                            <div id="signer-2" class="btn btn-light-yellow margB20 dragMe dragSigners" data-signer-id="2" data-document-id="1">
                                <span class="icon-menu pull-left"></span><spring:message code="label.date"/><span class="icon-menu pull-right"></span>
                            </div>

                            <div id="signer-3" class="btn btn-light-Bermuda margB20 dragMe dragSigners" data-signer-id="3" data-document-id="1">
                                <span class="icon-menu pull-left"></span><spring:message code="label.signature"/><span class="icon-menu pull-right"></span>
                            </div>
                        </div>
                        
                    </div>
                </div>
                <div id="pollSlider-button" style="margin-right: 0px;"><span class="icon-arrow_left"></span>
                <div class="rotedtext"><spring:message code="label.clickhereAddFields"/></div>
                </div>
            </div>
            <!--<div class="PDF_arrow">
    ...
    <div class="footer ">
        <div class="row ">
            <div class="col-xs-12 text-center ">
              <!-- <span class="icon-language"></span> ENGLISH  |  --> &copy;                 <script>
                    document.write(new Date().getFullYear())
                </script> <spring:message code="label.company"/>
            </div>
        </div>
    </div>
				...
				if(response == '103'){
					...
					//	for (var i = 1; i <= numpages; i++) {
								...
								$.ajax({
									...
									//	$("#document-reader").append('<div class="drop" id="'+i+'"><img width="'+width+'px" height="'+height+'px" class="render-image" onload="loadImage()" src="data:image/png;base64,'+<c:out value="response"/>+'"></div>');
										...
										if(response.indexOf('allPageImageJson')==-1){
											//alert("Calling getAllPages() with pdfInfo2 :" + JSON.stringify(pdfInfo2)); 
											getAllPages(contextPath,fileid,filename,userId,numpages,docId,header,width,height,pdfInfo2,token);
										}
										else{
											
											var json=JSON.parse(response);
												if(json.allPageImageJson==true){
												apppendAllpageJsonImgtoDoucment(docId,width,height,response,numpages,pdfInfo2);
												}
											}
...
function getAllPages(contextPath,fileid,filename,userId,numpages,docId,header,width,height,pdfInfo2,token){
	...
	for (var i=1; i <= numpages; i++) {
		...
		$.ajax({
			...
			success : function(response) {	
				//alert('<div class="drop" id="'+i+'"><img width="' + pdfInfo2.widths[i-1] +'px" height="' + pdfInfo2.heights[i-1] + 'px" class="render-image" onload="loadImage()" src="data:image/png;base64,'+<c:out value="response"/>+'"></div>');
				//old $("#document-reader").append('<div class="drop" id="'+i+'"><img width="'+width+'px" height="'+height+'px" class="render-image" onload="loadImage()" src="data:image/png;base64,'+<c:out value="response"/>+'"></div>');
				$("#document-reader").append('<div class="drop" id="'+i+'"><img width="' + pdfInfo2.widths[i-1] +'px" height="' + pdfInfo2.heights[i-1] + 'px" class="render-image" onload="loadImage()" src="data:image/png;base64,'+<c:out value="response"/>+'"></div>');
			},
			error : function(response) {
				$("#document-reader").append('<div class="drop" id="'+i+'"><img width="'+width+'px" height="'+height+'px" class="render-image" onload="loadImage()" src="data:image/png;base64,'+<c:out value="response"/>+'"></div>');
			},
...
function DragSigner() {
          ...
          $(".drop").droppable({
              ...
              drop: function(e, ui) {
                ...
                //alert("(ui.offset.left,top:" + ui.offset.left + "," + ui.offset.top +
                //") ($(this).offset().left,top:" + off.left + "," + off.top + "))");
	...
	$.ajax({
        ...
        success : function(response) {
        	if(response == '200'){ 
            	...
            	$("#success-messgae").html('<spring:message code="label.sucessfullyDocument"/>');
            }else if(response == '201'){
            	//alert('ERROR CODE: 201'); //KEY IS NOT EXIST 
            	$("#open_document_content").hide();
            	$("#error_content").show();
            	$("#error-messgae").html('<spring:message code="label.error.201"/>');
            }else if(response == '202'){
            	//alert('YOU HAVE ALREDY PREPARE THE DOCUMENT');
            	$("#open_document_content").hide();
            	$("#error_content").show();
            	$("#error-messgae").html('<spring:message code="label.error.202"/>');
            }else if(response == '203'){
            	//alert('ERROR CODE: 103'); //KEY IS NULL
            	$("#open_document_content").hide();
            	$("#error_content").show();
            	$("#error-messgae").html('<spring:message code="label.error.203"/>');
            }else if(response == '204'){
            	//alert('ERROR CODE: 104');  //ERROR WHILE SAVE FIELDS
            	$("#open_document_content").hide();
            	$("#error_content").show();
            	$("#error-messgae").html('<spring:message code="label.error.204"/>');
            }
		},
		 error : function(e) {  
		    alert('Error: ' + e);   
...
function qualityImprove(val){
	...
	//alert("i:" + i);
//	console.log('contextPath :'+contextPath+' userId :'+userId+' docId :'+fileId+' fileName :'+fileName+' numpages :'+numpages+' pdfInfo :'+ JSON.stringify(pdfInfo));
	...
	$.ajax({
		...
		$('#'+i).replaceWith( '<div class="drop" id="'+i+'"><img width="' + pdfInfo.widths[i-1] +'px" height="' + pdfInfo.heights[i-1] + 'px" class="render-image" onload="loadImage()" src="data:image/png;base64,'+<c:out value="response"></c:out>+'"></div>' );
		DragSigner();
	    DragMe();
		},
		error : function(response) {
